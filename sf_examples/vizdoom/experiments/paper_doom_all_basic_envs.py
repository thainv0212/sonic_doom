...
for env in envs:
    _experiments.append(
        Experiment(
            "basic_envs_fs4_no_sound",
            f"python -m sf_examples.vizdoom.train_vizdoom --train_for_env_steps=500000000 --algo=APPO --env_frameskip=4 --use_rnn=True --num_workers=36 --num_envs_per_worker=8 --num_policies=1 --batch_size=2048 --wide_aspect_ratio=False --env={env}",
            _params.generate_params(randomize=False),
        )
    )
    _experiments.append(
        Experiment(
            "basic_envs_fs4_vision_raw",
